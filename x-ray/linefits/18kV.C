{
gSystem->RedirectOutput("../outputs/linefits/txt/linefits.txt", "a");

cout << "\n------------------------------------------------------------------------------" << endl;
cout << "\n18kV\n" << endl;

ifstream f; 
f.open("../data/18kV.txt");
...
TF1* linefit = new TF1("linefit", "[0]*x+[1]", 0.060, 0.1);
linefit->SetLineWidth(5);
linefit->SetLineColor(kRed);
linefit->SetMarkerStyle(8);
data->Fit(linefit, "R", "", 0.066, 0.078);
linefit->Draw("SAME");
...
c1->SaveAs("../outputs/linefits/images/svg/18kV.png");
c1->SaveAs("../outputs/linefits/images/pdf/18kV.pdf");
c1->Close();
